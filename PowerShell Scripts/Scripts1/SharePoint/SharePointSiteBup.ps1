## Currently running on CAMRY - Task Scheduler Library / Microsoft / Windows - Weekly Saturday 1AM
...
$BupSites = gc e:\temp\spsitesII.txt
	foreach ($sharepointsite in $BupSites)
		{
			stsadm -o backup -url http://intranet/sites/$sharepointsite -filename \\powc-dxi-02-pp\backup_01\1SharePointSites\$sharepointsite"_"$BUPdate.bak
		}


stsadm -o backup -url http://intranet -filename \\powc-dxi-02-pp\backup_01\1SharePointSites\M1_Intranet"_"$BUPdate.bak
...
$BupSitesII = gc e:\temp\spsites_sodintranetII.txt
	foreach ($sharepointsiteII in $BupSitesII)
		{
			stsadm -o backup -url http://sodintranet/$sharepointsiteII/home -filename \\powc-dxi-02-pp\backup_01\1SharePointSites\$sharepointsiteII"_"$BUPdate.bak
		}


stsadm -o backup -url http://sodintranet -filename \\powc-dxi-02-pp\backup_01\1SharePointSites\SOD_Intranet"_"$BUPdate.bak
stsadm -o backup -url http://sodintranet/sites/ducmgmt -filename \\powc-dxi-02-pp\backup_01\1SharePointSites\ducmgmt"_"$BUPdate.bak
