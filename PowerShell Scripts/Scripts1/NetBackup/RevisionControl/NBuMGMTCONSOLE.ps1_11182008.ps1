...
Elseif ($listBox1.SelectedItem -eq "ACTIVE JOBS")
{
$GactiveJobs = psexec \\gremlin "D:\program files\veritas\netbackup\bin\admincmd\bpdbjobs" -ignore_parent_jobs -noheader |
	...
	#where-object {$_-match "Active" -and $_-notmatch "Done" -and $_-notmatch "Duplication" }  | 
	...
	#foreach-object {$_ -replace "Duplication,","Duplication,NOP,"","} |	
	Foreach-object {$_+"`n"}
	
	$richtextbox1.font = "courier"
	$richtextbox1.text = ""
	$temptype = New-Object string[] 9
	foreach ($object in $GactiveJobs)
	{
		for ($i =0;$i -lt 9;$i++) 
		{
	 		$temptype[$i] = $object.split(",",[StringSplitOptions]::RemoveEmptyEntries)[$i]
			
						
			#$type = $type + $temptype
			
		}
		$out = "{0,-11} {1,-38} {2,-5} {3,-12} {4,-10} {5,-10} {6,-12} {7,10} {8,10}"-f $temptype[0],$temptype[1],$temptype[2],$temptype[3],$temptype[4],$temptype[5],$temptype[6],$temptype[7],$temptype[8]
		#$richTextBox1.Text = $richTextBox1.Text + $out
	}
...
$listBox1.Items.Add('ACTIVE JOBS')|Out-Null
