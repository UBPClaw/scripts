...
Remove-Item D:\BKD_LOGS\CISBUPS\CISBUPTempLogs.txt                                  
#Remove-Item D:\BKD_LOGS\CISBUPS\NBULogsToSQL.txt
Remove-Item D:\BKD_LOGS\CISBUPS\CISBUPTempLogs1.txt
...
$BeforeDt = (get-date (get-date).AddDays(-1) -f M/d/yyyy)
$BeforeFileDate = $BeforeDt | ForEach-Object {$_-replace "\/","_"}
$BeforeDtDay = (get-date (get-date).AddDays(-1) -f dddd)
...
# Run the NBU command and assign it a variable of $Logs.
 ...
 $Logs1 = bpdbjobs -ignore_parent_jobs -most_columns 
 
 $logs = $logs1 | 
 Where-Object {$_-match "NT_CIS_Before_DayEnd_DD1" -or $_-match "NT_CIS_After_DayEnd_DD1"}
...
# The split method allows us to grab data at a specific comma.
		...
		$BupCopy = $line.split(",")[35]
			if ($BupCopy -eq "")
				...
				# After scraping away what we need, recompile it into a comman delimited line of text and then output it
				 ...
				 "," + $BupStart + "," + $BupEnd + "," + $StrgUnt + "," + $KBS + "," + $Files + "," + $BupID + "," + $BupCopy | Out-File D:\BKD_LOGS\CISBUPS\CISBUPTempLogs1.txt -append
				 
				 
	}
 
 
 gc D:\BKD_LOGS\CISBUPS\CISBUPTempLogs1.txt |

               foreach {$_-replace "Gremlin-Nova","TAPE"} |
			   foreach {$_-replace "Nova-LTO2","TAPE"} |
			   foreach {$_-replace "Gremlin-LTO2","TAPE"} | Out-File D:\BKD_LOGS\CISBUPS\CISBUPTempLogs.txt -append
 ...
 gc D:\BKD_LOGS\CISBUPS\CISBUPTempLogs.txt | Where-Object {$_.split(",")[8] -match $BeforeFileDate -and $_.split(",")[11] -ne "0"-and $_.split(",")[14] -lt "2" } | Out-File D:\BKD_LOGS\CISBUPS\CISBUP_Before.txt -append
