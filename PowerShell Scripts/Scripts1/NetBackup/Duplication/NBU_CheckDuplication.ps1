...
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\checking\NeedDuped_$RemoveChar.txt
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\checking\DupCheck_$RemoveChar.txt
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\checking\Duped_$RemoveChar.txt
...
$client = get-childitem -Name $gpath | where-object {$_-match "HDR" -and $_-notmatch ".img"} 
		...
		foreach ($item in $client)
		{
			...
			$2 = $item.split("_",[StringSplitOptions]::RemoveEmptyEntries)[1] 
		        $final = $1 + "_" + $2
			$final | Out-File D:\BKD_LOGS\Duplication\DuplicateThis\checking\DupCheck_$RemoveChar.txt -append
			}
...
$BupID = gc D:\BKD_LOGS\Duplication\DuplicateThis\checking\DupCheck_$RemoveChar.txt
...
foreach ($Copy in $BupID )
	...
	$gcopynum = bpimagelist -l -backupid $Copy | 
		...
		if ($gcopynum  -gt 1)
		{		
		    $Copy + " " + $gcopynum | Out-file D:\BKD_LOGS\Duplication\DuplicateThis\checking\Duped_$RemoveChar.txt -append
			}
			else
			{
				$Copy + " " + $gcopynum | Out-file D:\BKD_LOGS\Duplication\DuplicateThis\checking\NeedDuped_$RemoveChar.txt -append
