...
Remove-Item \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\AllImagesDUser.txt
Remove-Item \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistDUser.txt
...
#$Header | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationDUser.txt -append
...
# Build the Policy Active/Not Active list and export it as a CSV
 ...
 $PolList = bppllist |
 	where-object {$_-match "NA_Dakota_User_OffSite"} 
...
foreach ($Pol2 in $PolList)
	...
	$image = bpimagelist -policy $Pol2 -d $DlyBupDt -e $Today 23:59:59|
	   Where-Object {$_-match "IMAGE"}  | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\AllImagesDUser.txt -append
...
$GBupID = gc \\gremlin\d$\bkd_logs\Duplication\VerifyOn_Offsite\AllimagesDuser.txt 
				...
				foreach ($object in $Img2)
					{
						...
						$final | Sort-Object -Unique | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistDUser.txt -append
					}
...
# and the MediaID\Barcode into two seperate variables. We will still need the policy name when we are putting this baby back together.
		...
		$Gimages = gc \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistDUser.txt | Sort-Object -Unique  
...
# Output The Backup Date, Policy name, Barcode, Volume Pool, and the Experation Date to a csv file. 			
		...
		$jobidf + "," + $BDate + "," + $Bpolicy + "," + $Bcode + "," + $MedPool + "," + $TapeExpire | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationDUser.txt -append
...
# file then import it and then export it. I would think we could just export it from the beginning. Will follow up on this one.
		...
		# Import-Csv \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationDUser.txt | sort-object bupdate, jobid -Descending | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\DakotaUserLocationRpt.htm -force
		 # Import-Csv \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationDUser.txt | sort-object bupdate -Descending | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\DakotaUserLocationRpt.htm -force
		  Import-Csv \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationDUser.txt | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\DakotaUserLocationRpt.htm -force