...
Remove-Item \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistAllImagesOneOff.txt
Remove-Item \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\AllImagesOneOff.txt
Remove-Item \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationAllImagesOneOff.txt
...
$Header | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationAllImagesOneOff.txt


# Build the Policy Active/Not Active list and export it as a CSV
# Change the policy to the desired policy.


 $PolList = "NA_Dakota_Dept_OffSite"
 
 


$DlyBupDt = (get-date (get-date).AddDays(-15) -f M/d/yyyy)
...
foreach ($Pol2 in $PolList)
	...
	$image = bpimagelist -policy $Pol2 -d $DlyBupDt -e $Today 23:59:59|
	   Where-Object {$_-match "IMAGE" -and $_-match "Weekly"}  | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\AllImagesOneOff.txt -append
...
$GBupID = gc \\gremlin\d$\bkd_logs\Duplication\VerifyOn_Offsite\AllImagesOneOff.txt 
				...
				foreach ($object in $Img2)
					{
						...
						$final | Sort-Object -Unique | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistAllImagesOneOff.txt -append
					}
...
# and the MediaID\Barcode into two seperate variables. We will still need the policy name when we are putting this baby back together.
		...
		$Gimages = gc \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\ImagelistAllImagesOneOff.txt | Sort-Object -Unique  
...
# Output The Backup Date, Policy name, Barcode, Volume Pool, and the Experation Date to a csv file. 			
		$jobidf + "," + $BDate + "," + $Bpolicy + "," + $Bcode + "," + $MedPool + "," + $TapeExpire | Out-File \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationAllImagesOneOff.txt -append
...
# file then import it and then export it. I would think we could just export it from the beginning. Will follow up on this one.
		Import-Csv \\gremlin\d$\BKD_LOGS\Duplication\VerifyOn_Offsite\TapeLocationAllImagesOneOff.txt | sort-object bupdate, jobid -Descending | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\OneOffReport.htm -force