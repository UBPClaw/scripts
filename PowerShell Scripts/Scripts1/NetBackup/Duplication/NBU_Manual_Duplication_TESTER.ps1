...
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\NeedDuped_$dupgood.txt
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\DupCheck_$dupgood.txt
Remove-Item D:\BKD_LOGS\Duplication\DuplicateThis\Duped_$dupgood.txt
...
$client = get-childitem -Name $gpath | where-object {$_-match "HDR" -and $_-notmatch ".img"} 
		...
		foreach ($item in $client)
		{
			...
			$final = $1 + "_" + $2 | Out-File	D:\BKD_LOGS\Duplication\DuplicateThis\DupCheck_$dupgood.txt -append
			}
...
$BupID = gc D:\BKD_LOGS\Duplication\DuplicateThis\DupCheck_$dupgood.txt
...
foreach ($Copy in $BupID )
	...
	#$gcopynum = psexec \\gremlin "D:\program files\veritas\netbackup\bin\admincmd\bpimagelist" -l -backupid $Copy | 
		Where-Object {$_-notmatch "FRAG"} | 
		Where-Object {$_-notmatch "HISTO"} | 
		foreach { $_.split(" ",[StringSplitOptions]::RemoveEmptyEntries)[20]}
		if ($gcopynum  -gt 2)
		{		
		    $Copy + " " + $gcopynum | Out-file D:\BKD_LOGS\Duplication\DuplicateThis\Duped_$dupgood.txt -append
			}
			else
			{
				$Copy + " " + $gcopynum | Out-file D:\BKD_LOGS\Duplication\DuplicateThis\NeedDuped_$dupgood.txt -append
			     
				 
				 
				 
				 bpduplicate -backupid $copy -dstunit Gremlin-Nova -dp Offsite
