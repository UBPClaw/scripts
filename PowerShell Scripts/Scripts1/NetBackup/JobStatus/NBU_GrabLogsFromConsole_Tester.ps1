...
#
# NAME: NBU_GrabLogsFromConsole.ps1 *Test Script
#
...
Remove-Item D:\BKD_LOGS\NBULOGS\NbuTempLogsTester.txt                                  
#Remove-Item D:\BKD_LOGS\NBULOGS\NBULogsToSQL.txt
Remove-Item D:\BKD_LOGS\NBULOGS\NbuTempLogsTester1.txt
#Remove-Item D:\BKD_LOGS\NBULOGS\DriveDetails.txt
...
# The split method allows us to grab data at a specific comma.
		...
		$BupCopy = $line.split(",")[35]
			if ($BupCopy -eq "")
				...
				# After scraping away what we need, recompile it into a comman delimited line of text and then output it
				 ...
				 "," + $BupStart + "," + $BupEnd + "," + $StrgUnt + "," + $KBS + "," + $Files + "," + $BupID + "," + $BupCopy | Out-File D:\BKD_LOGS\NBULOGS\NbuTempLogsTester1.txt -append
				 
				 
	}
 
 
 gc D:\BKD_LOGS\NBULOGS\NbuTempLogsTester1.txt |

               foreach {$_-replace "Gremlin-Nova","TAPE"} |
			   foreach {$_-replace "Nova-LTO2","TAPE"} |
			   foreach {$_-replace "Gremlin-LTO2","TAPE"} | Out-File D:\BKD_LOGS\NBULOGS\NbuTempLogsTester.txt -append
 ...
 gc D:\BKD_LOGS\NBULOGS\NbuTempLogsTester.txt | Where-Object {$_.split(",")[3] -eq "0" -or $_.split(",")[3] -eq "1" -and $_.split(",")[1] -eq "0" -and $_.split(",")[8] -match $BupDt}| Out-File D:\BKD_LOGS\NBULOGS\NBULogsToSQLTester_$FileDate.txt -append
 ...
 Rename-Item D:\BKD_LOGS\NBULOGS\NbuTempLogsTester.txt NBULOGSTester_$FileDate.txt
...
$JobIDScrape = gc D:\BKD_LOGS\NBULOGS\"NBULogsToSQLTester_"$FileDate.txt |
foreach { $_.split(",")[0]}
...
foreach ($JobID in $JobIDScrape)

	{
			bperror -hoursago 168 -jobid $JobID -l |
				...
				Out-File "D:\BKD_LOGS\NBULOGS\DriveDetailsTester.txt" -append
	$JobID
	}
	
	 gc D:\BKD_LOGS\NBULOGS\DriveDetailsTester.txt |sort | Get-Unique |Out-File D:\BKD_LOGS\NBULOGS\"DriveDetailsTester_"$FileDate.txt
