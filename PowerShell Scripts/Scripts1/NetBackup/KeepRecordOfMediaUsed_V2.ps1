...
#$Header = "Date,   Tapes Used"
#$Header | Out-File \\gremlin\d$\temp\MediaCount2.txt -append
...
remove-Item \\gremlin\d$\temp\MediaList2.txt  # This is the orignal out put of the NBU command bpmedialist
remove-Item \\gremlin\d$\temp\media2.txt      # This text file contains the date only.
...
bpmedialist -L  > \\gremlin\d$\temp\MediaList2.txt
...
$Findate2 = (get-date (get-date).AddDays(-1) -f D)
$Findateout = $Findate2 | ForEach-Object {$_-replace ", ","_"} 
...
$mlist = get-content \\gremlin\d$\temp\media2.txt | where-object {$_-match $findate}
$mcount = $mlist.count
$Mfin = $Findateout + ",********** " + $mcount | out-file \\gremlin\d$\temp\MediaCount2.txt -append
#Import-Csv \\gremlin\d$\temp\MediaCount2.txt | sort-object date -Descending | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\MediaCountRPT.htm -force
Import-Csv \\gremlin\d$\temp\MediaCount2.txt | ConvertTo-HTML | out-file \\malibu\it\docs\NetBackup\MediaMonitor\MediaCountRPT.htm -force
