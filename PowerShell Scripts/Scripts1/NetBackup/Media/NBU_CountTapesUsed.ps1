...
Remove-Item D:\BKD_LOGS\Media\CountTapes\Policies\*.*
Remove-Item D:\BKD_LOGS\Media\CountTapes\Final\*.*
...
# Build the Policy Active/Not Active list and export it as a CSV
 ...
 #$PolList = bppllist|
 	#where-object {$_-eq "NA_Dakota_User" } 
				...
				foreach ($Line in $Img)
					{
						...
						$UnixDate = $Line.split(" ",[StringSplitOptions]::RemoveEmptyEntries)[12]
...
# Output The Backup Date, Policy name, Barcode, Volume Pool, and the Experation Date to a csv file. 			
		...
		$BDate + "," + $Bpolicy + "," + $PSsched2 + ","+ $Bcode + "," + $MedPool + "," + $TapeExpire | Out-File D:\BKD_LOGS\Media\CountTapes\policies\$Bpolicy.txt -append
		 
		          }


$GetPfiles = get-childitem D:\BKD_LOGS\Media\CountTapes\Policies -name

foreach ($GPF in $GetPfiles)
	{
		$CountTapes = gc D:\BKD_LOGS\Media\CountTapes\Policies\$GPF
			$GPF + "," + @($CountTapes).count | out-file D:\BKD_LOGS\Media\CountTapes\Final\PreFinal.txt -append
...
$GetPfinal = gc D:\BKD_LOGS\Media\CountTapes\final\PreFinal.txt |

ForEach-Object {$_-replace ".txt",""} | out-file D:\BKD_LOGS\Media\CountTapes\final\PolTapUsage.txt
